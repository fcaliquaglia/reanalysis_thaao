...
buoy_files = sorted(glob.glob(os.path.join(folder_path_buoys, '2024*processed.csv')))
...
for buoy_file in buoy_files:
    ...
    df = df.dropna(subset=['time_stamp', 'latitude', 'longitude']).reset_index(drop=True)
    ...
    ds = xr.Dataset(
        data_vars=dict(
            ...
            air_pressure=(["trajectory", "obs"], expand_dim(df["air_pressure"])),
            Surface_Position=(["trajectory", "obs"], expand_dim(df["Surface_Position"])),
            Bottom_Position=(["trajectory", "obs"], expand_dim(df["Bottom_Position"])),
            snow_depth=(["trajectory", "obs"], expand_dim(df["snow_depth"])),
            Ice_Thickness=(["trajectory", "obs"], expand_dim(df["Ice_Thickness"])),
            water_temp=(["trajectory", "obs"], expand_dim(df["water_temp"])),
            surface_distance=(["trajectory", "obs"], expand_dim(df["surface_distance"])),
            bottom_distance=(["trajectory", "obs"], expand_dim(df["bottom_distance"])),
            slip_offset=(["trajectory", "obs"], expand_dim(df["slip_offset"])),
            Temp_Derived_Surface_Position=(["trajectory", "obs"], expand_dim(df["Temp-Derived_Surface_Position"])),
            Ice_Surface_Position=(["trajectory", "obs"], expand_dim(df["Ice_Surface_Position"])),
        ),
        coords=dict(
        ...
        attrs=dict(
            ...
            Conventions="CF-1.8",
            institution="Generated by Filippo Calì Quaglia"
        )
    )
    ...
    ds['time'].attrs['units'] = 'seconds since ' + df['time_stamp'][0].strftime('%Y-%m-%d %H:%M:%S')
