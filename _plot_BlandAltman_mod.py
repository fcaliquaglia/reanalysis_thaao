...
def blandAltman(data1, data2, limitOfAgreement=1.96, confidenceInterval=95, confidenceIntervalMethod='approximate', percentage=False, detrend=None, title=None, ax=None, i=None, ctrl=0, fig=plt.Figure(), figureSize=(10, 7), dpi=72, savePath=None, figureFormat='png', meanColour='#6495ED', loaColour='coral', pointColour='#6495ED'):
    """
    ...
    if not limitOfAgreement > 0:
        raise ValueError(
            '"limitOfAgreement" must be a number greater than zero.')
    ...
    if confidenceInterval:
        confidenceIntervals = calculateConfidenceIntervals(md, sd, len(
            diff), limitOfAgreement, confidenceInterval, confidenceIntervalMethod)
...
def _drawBlandAltman(mean, diff, md, sd, percentage, limitOfAgreement, confidenceIntervals, detrend, title, axs, i, ctrl, fig, figureSize, dpi, savePath, figureFormat, meanColour, loaColour, pointColour):
    """
    ...
    ##
    # ax.scatter(mean, diff, alpha=0.5, c=pointColour, marker='.')
    ##

    var_data = inpt.extr[inpt.var]
    # === Replace scatter with 2D histogram ===
    ...
    # Generate colormap (white lowest color)
    ...
    # Create bin edges
    bin_nr = var_data.get('bin_nr', 1)
    bin_edges = np.linspace(vmin, vmax, bin_nr)
    bin_size = (vmax - vmin) / bin_nr
    ...
    ax.text(0.98, md + offset, 'Mean', ha="right",
            va="bottom", transform=trans)
    ax.text(0.98, md - offset, f'{md:.2f}',
            ha="right", va="top", transform=trans)

    ax.text(0.98, md + (limitOfAgreement * sd) + offset,
            f'+{limitOfAgreement:.2f} SD', ha="right", va="bottom", transform=trans)
    ax.text(0.98, md + (limitOfAgreement * sd) - offset,
            f'{md + limitOfAgreement*sd:.2f}', ha="right", va="top", transform=trans)

    ax.text(0.98, md - (limitOfAgreement * sd) - offset,
            f'-{limitOfAgreement:.2f} SD', ha="right", va="top", transform=trans)
    ax.text(0.98, md - (limitOfAgreement * sd) + offset,
            f'{md - limitOfAgreement*sd:.2f}', ha="right", va="bottom", transform=trans)
    ...
    labels = rangeFrameLabler(ax.xaxis.get_ticklocs(), [
                              item.get_text() for item in ax.get_xticklabels()], cadenceX)
    ax.set_xticklabels(labels)

    labels = rangeFrameLabler(ax.yaxis.get_ticklocs(), [
                              item.get_text() for item in ax.get_yticklabels()], cadenceY)
    ...
    if detrend[0] is None:
        ...
        plt.text(1, -0.1, f'{detrend[0]} slope correction factor: {detrend[1]:.2f} ± {detrend[2]:.2f}',
